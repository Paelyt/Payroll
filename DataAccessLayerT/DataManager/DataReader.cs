...
namespace DataAccessLayerT.DataManager
{
    public class DataReader
    {
        ...
        static string cn = ConfigurationManager.AppSettings["ConnectionStringData"];
        ...
        public static DataView GetInvoicePaid(int myMonth, int myYear, int InstitutionFk)
        {
            DataSet ds = null;
            try
            {

                SqlParameter[] @params = { new SqlParameter("@myMonth", myMonth),
                                new SqlParameter("@InstitutionFk", InstitutionFk),
                                        new SqlParameter("@myYear", myYear)};
                ds = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "GetInvoicePaid", @params);
                if (ds == null)
                {
                    return null;
                }
            ...
            {
                WebLog.Log(ex.Message, ex.StackTrace.ToString());
                return null;
            }
        }
        ...
        public float GetAbandonrate()
        {
            try
            {
                ...
                var AbandonApplication = (from a in uvDb.PatnerTransactLogs
                                          join b in uvDb.LoanApplications
        on a.RefNum equals b.LoanRefNumber
                                          where DateCap > System.Data.Entity.DbFunctions.DiffDays(DateTime.Now, a.DateCreated)
                                          select a).Count();
        ...
        public int CountDocsForReview(string Refid, out string respMessage)
        {
            try
            {
                var recVal = (from a in uvDb.DocUploads where a.ReferenceNum == Refid select a).ToList();

                //var trxx = uvDb.DocUploads
                //    .Where(x => x.ReferenceNum == Refid).ToList();

                if (recVal == null || recVal.Count == 3)
                {
        ...
        public List<AppLoan> GetDocument(int Users, string RefNum)
        {
            try
            {
                var records = (from a in uvDb.DocUploads
                               join b in uvDb.LoanApplications on a.ReferenceNum equals b.LoanRefNumber
                               where a.User_FK == Users && a.ReferenceNum == RefNum
                               select new AppLoan
                               {
        ...
        public Institution getInstitution(int instFK)
        {
            try
            {
                var record = (from a in uvDb.Institutions where a.ID == instFK select a).FirstOrDefault();
        ...
        public AppLoan StudentLoanDetails(string Refid)
        {
            ...
            {
                var LoanDetails = (from a in uvDb.LoanApplications
                                   join b in uvDb.StudentLoanDetails on a.ID equals b.LoanApplication_FK
                                   join c in uvDb.ApplicationStatus on
a.ApplicationStatus_FK equals c.ID
                                   where a.LoanRefNumber == Refid
        ...
        public DataAccessLayerT.Classes.LoanApplication getApplicantEmpInfo(int id)
        {
            try
            {
                var EmpInfo = (from a in uvDb.LoanApplications
                               join b in uvDb.EmployerLoanDetails on a.ID equals b.LoanApplication_FK
                               join c in uvDb.Institutions on a.Institution_FK equals c.ID
                               where a.ID == id
        ...
        public List<Invoice> getAllLedRecordsPaid(string RefNum, DateTime startDate)
        {
            try
            {
                var Invoice = (from a in uvDb.LoanApplications
                               ...
                               where c.TranxDate.Value.Month <= startDate.Month && c.TranxDate.Value.Year <= startDate.Year && c.IsVisible == 1 && c.PaymentFlag_FK > 0
                               && c.RefNumber == RefNum
                               select new Invoice
                               {
                                   LID = (int)c.ID,
        ...
        public List<Invoice> getAllLedRecordPaid(string RefNum, DateTime startDate)
        {
            try
            {
                var Invoice = (from a in uvDb.LoanApplications
        ...
        public List<Invoice> getInvoiceV0(DateTime startDate, DateTime EndDate, int InstitutionFk)
        {
            try
            {
                var Invoice = (from a in uvDb.LoanApplications
        ...
        public List<Invoice> getInvoicePaid(DateTime startDate, DateTime EndDate, int InstitutionFk)
        {
            try
            {
                List<Invoice> invList = new List<Invoice>();

                DataView dv = GetInvoicePaid(startDate.Month, startDate.Year, InstitutionFk);
                ...
                for (int i = 0; i < dv.Count ; i++)
                {
                    ...
                    invoice.LID = Convert.ToInt16(dv[i]["LID"]);
                    invoice.Company = dv[i]["Company"].ToString();
                    invoice.Location = dv[i]["Location"].ToString();
                    invoice.Fullname = dv[i]["Fullname"].ToString();
                    invoice.DisburseDate =Convert.ToDateTime( dv[i]["DisburseDate"]);
                    invoice.RepaymentDate = Convert.ToDateTime(dv[i]["RepaymentDate"]);
                    invoice.Interestrate = Convert.ToInt16(dv[i]["Interestrate"]);
                    invoice.Tenure = Convert.ToInt16(dv[i]["Tenure"]);
                    invoice.Debit = Convert.ToDouble(dv[i]["InterestPricipalDue"]);
                    invoice.InterestPricipalDue = Convert.ToDouble(dv[i]["InterestPricipalDue"]);
                    invoice.ReferenceNum = dv[i]["ReferenceNum"].ToString();
                    invoice.Credit = Convert.ToDouble(dv[i]["Credit"]);
                    invoice.EmployeeID = dv[i]["EmployeeID"].ToString();
                    invoice.PaymentDate = startDate.ToString("yyyy/MM/dd");
                    invList.Add(invoice);
                }
                HttpContext.Current.Session["Invoicevav"] = dv;
        ...
        public LoanInterestRate GetRate(int tenure, string InstCode)
        {
            try
            {
                var InterestRate = (from a in uvDb.LoanInterestRates
                                    ...
                                    where a.LoanTenure == tenure && b.InstitutionCode == InstCode
        ...
        public List<LoanApplication> GetLoans(int instFK, int appSTatus)
        {
            try
            {
                ...
                var Loans = (from a in uvDb.LoanApplications
                             join b in uvDb.Institutions on a.Institution_FK equals b.ID
                             where a.IsVisible == 1 && a.ApplicationStatus_FK == appSTatus && b.InstitutionType_FK == 2 && a.Institution_FK == instFK
                             select a).ToList();
        ...
        public List<Classes.LoanApplication> GetAllRemitaTrans(int instFK)
        {
            try
            {
                ...
                var Loans = (from a in uvDb.PatnerTransactLogs
                             where a.PatnerCode == "Remita"
                             select new Classes.LoanApplication
                             {
...
d.Code
    ...
    d.Code
        ...
        public LoanLedger GetNextLoanLedger(LoanLedger loanLedger)
        {
            try
            {
                //  var loanLedger = (from a in uvDb.LoanLedgers where a.ID == idv select a).FirstOrDefault();

                if (loanLedger == null)
                {
                    return null;
                }
                DateTime nextTranxDate = loanLedger.TranxDate.Value.AddMonths(1);
                int yearVal = nextTranxDate.Year;
                int monthVal = nextTranxDate.Month;
                var loanRef = loanLedger.RefNumber;
                var loanLedger1 = (from a in uvDb.LoanLedgers where a.RefNumber == loanRef && a.TranxDate.Value.Year == yearVal && a.TranxDate.Value.Month == monthVal select a).FirstOrDefault();
                if (loanLedger1 == null)
                {
                    return null;
                }
        ...
        public double GetAmountPaid(int idv, out double balance)
        {
            balance = 0;
            try
            {
                var AmountPaid = (from a in uvDb.LedgerTransacts where a.colss == idv select a.Credit).Sum();
                var AmountOwed = (from a in uvDb.LoanLedgers where a.ID == idv select a.Debit).Sum();
                ...
                balance = AmountOwed.Value - AmountPaid.Value;
                return AmountPaid.Value;
            }
            catch (Exception ex)
            {
...
a.LoanTenure == tenure
        ...
        public List<LoanLedger> getNewCustomer(string Email)
        {
            try
            {
                ...
                using (var context = new UvlotEntities())
                {
                    InstEmail = uvDb.Database.SqlQuery<LoanLedger>("SELECT * from [LoanLedger] u  WHERE u.RefNumber = '" + Email + "' ORDER BY TranxDate  ").ToList();
                }
                if (InstEmail == null)
                {
                    return null;
        ...
        public List<LoansLeger> GetLoanLedger(string RefNumber)
        {
            try
            {
                var LoanLedger = (from a in uvDb.LoanLedgers
                                  join b in uvDb.LoanApplications on a.RefNumber equals b.LoanRefNumber
        ...
        public List<DataAccessLayerT.DataManager.AppLoan> getSignature(string Refid)
        {
            try
            {
                var AppLoan = (from a in uvDb.Signatures
                               where a.LoanRefNum == Refid
                               select new DataAccessLayerT.DataManager.AppLoan
                               {
        ...
        public int GetInstitutionByName(string Institution, out string instCode)
        {
            instCode = "";
            try
            {
                ...
                instCode = InstitutionName.InstitutionCode;
                return InstitutionName.ID;
            }
            catch (Exception ex)
            {
